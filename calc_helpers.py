...
def calc_frame(frame: Dict[str, Any], glass_thk: float = 0, alum_profiles_data: list = None, steel_profiles: list = None) -> Optional[Dict[str, Any]]:
    ...
    tran_vu = round(1.6 * tran_w_wind * (frame_width / 1000) / 4, 2)
    ...
    if frame_type == "Floor-to-floor":
        ...
        reaction_Ry = round(mul_w_wind * (frame_length / 1000) * (10 / 8), 2)
        reaction_Rz = mul_w_dead
    else:
        mul_vu = 0
        reaction_Ry = 0
        reaction_Rz = 0
...
def calc_anchorage(anchor: Dict[str, Any], frame: Dict[str, Any], glass_thk: float = 0, alum_profiles_data: list = None) -> Optional[Dict[str, Any]]:
    ...
    if frame_type == "Floor-to-floor":
        ...
        reaction_Ry = mul_w_wind * (frame_length / 1000) * (10 / 8)
    ...
    if clump_type == "U Clump":
        ...
        result.update({
            ...
            "fin_length": round(fin_length, 2),
            "fin_width": round(fin_width, 2),
    ...
    if clump_type == "L Clump":
        ...
        result.update({
            ...
            "psi_edN": round(top_psi_edN, 2),
            "N_b": round(top_N_b, 2),
            "psi_edV": round(top_psi_edV, 2),
            "psi_hV": round(top_psi_hV, 2),
            "l_e": round(l_e, 2),
            "V_b": round(top_V_b, 2),
            ...
            "top_bp_width": top_bp_width_B,
            "front_bp_length": front_bp_length_N,
            "front_bp_width": front_bp_width_B,
            "bp_lambda_n": round(bp_lambda_n, 2),
            "bp_l": round(bp_l, 2),
            "bp_q": round(bp_q, 2),
            "bp_bearing_Mu": round(bp_bearing_Mu, 2),
